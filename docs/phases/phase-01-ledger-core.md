...
> This document describes the ledger foundation. The current repository builds the Wallet, projection, messaging, Saga and observability layers on top of this foundation; statements below are scoped to the ledger phase.
...
The current implementation contains the application use cases, HTTP API, PostgreSQL repository, reversal flow, idempotency handling, and integration coverage for these capabilities.
...
All `Account` references in Phase 1 point to accounts owned by this Ledger. External bank accounts, beneficiaries, payment rails, and provider operations are outside the Phase 1 domain model.
...
IDs are UUID values generated by the application. The current implementation uses the Go UUID library; checkout aggregate IDs are deterministic for a given idempotency key so a retry can safely reuse the same hold, transaction and posting identifiers.

## API

The current HTTP API uses `net/http` handlers with `chi` routing and shared response/error helpers:
...
Errors expose a stable `code`, a human-readable `error`, and the request ID when available. The API accepts an `Idempotency-Key` for transaction posting and reversal.

## External Payment Boundary

External transfers are intentionally deferred. In a later phase, an operation involving another institution will use a local clearing or settlement account for the accounting facts and a separate payment instruction for the external destination. The external account will be represented by an external reference, not by a local `Account` row.

An external provider or payment rail may return an asynchronous operational status such as `pending`, `settled`, `failed`, or `rejected`. That status belongs to the future payment instruction. It MUST NOT mutate the original posted postings. A financial correction after a local posting requires a new compensating journal entry; if approval must precede movement, a future hold/authorization model may be used.

The provider, queue, callback, settlement, reconciliation, and compensating workflow are not implemented in Phase 1.
...
- Integration tests: application plus real PostgreSQL, covering transaction atomicity, constraints, idempotent replay, conflicting keys, reversal persistence, and query behavior.
- Concurrency tests cover posting, wallet withdrawals and hold authorization with explicit contention scenarios.

PostgreSQL is not mocked in integration tests. Docker Compose is the supported local development path and the integration suite uses the real PostgreSQL service.
...
- `go test ./...`, `go vet ./...`, and the documented local migration workflow pass.
- The README and architecture documents describe the implementation state accurately.

Handwritten SQL remains under version control together with the generated SQLC access code so schema changes are reviewable and reproducible.
