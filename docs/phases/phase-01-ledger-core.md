...
`sqlc generate` remains an environment/tooling issue on the current Windows setup; the handwritten SQL files remain the source of truth and the generated fallback is kept under version control until SQLC generation works reliably in the development environment.
